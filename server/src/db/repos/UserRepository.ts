...
class UserRepositoryInternal {
	...
	async findAll({ skip, take }: IPaginationArgs = {}, filter?: Prisma.UserWhereInput, orderBy?: Prisma.UserOrderByWithRelationInput): Promise<User[]> {
	...
	async findBannedUsers({ skip, take }: IPaginationArgs = {}): Promise<User[]> {
		return prisma.user.findMany({
			// Use imported `prisma`
			skip,
			take,
