...
export type CharacterCreateData = Omit<
  ...
  | 'ownedEntities'
> & { userId: string };
export type CharacterUpdateData = Partial<
  Omit<
    Prisma.CharacterUpdateInput,
    'id' | 'createdAt' | 'user' | 'inventoryEntities' | 'equipmentEntities' | 'ownedEntities'
  >
>;
...
class CharacterRepositoryInternal {
  ...
  ): Promise<CharacterWithUser | Character | null> {
  ...
  async updateExperience(
    id: string,
    experienceGained: number | bigint,
    newLevel?: number,
  ): Promise<Character> {
