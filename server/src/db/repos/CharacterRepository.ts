...
// PaginationArgs, CharacterCreateData, CharacterUpdateData, includes, and derived types remain the same
...
export type CharacterCreateData = Omit<Prisma.CharacterCreateInput, 'id' | 'createdAt' | 'lastPlayedAt' | 'playTime' | 'user' | 'inventoryEntities' | 'equipmentEntities' | 'ownedEntities'> & { userId: string; };
...
const characterIncludeUser = { user: { select: { id: true, username: true } } } satisfies Prisma.CharacterInclude<DefaultArgs>;
// const characterIncludeFull = { ... } // Define as before
export type CharacterWithUser = Prisma.CharacterGetPayload<{ include: typeof characterIncludeUser; }>;
...
class CharacterRepositoryInternal {
  ...
  async create(data: CharacterCreateData): Promise<Character> {
    return prisma.character.create({ // Use imported `prisma`
      data,
    });
  }

  async findById(id: string, includeUser: boolean = false): Promise<CharacterWithUser | Character | null> {
    return prisma.character.findUnique({ // Use imported `prisma`
      where: { id },
  ...
  async findByUserId(userId: string, { skip, take }: PaginationArgs = {}): Promise<Character[]> {
    return prisma.character.findMany({ // Use imported `prisma`
      where: { userId },
      skip,
      take,
      orderBy: {
  ...
  async update(id: string, data: CharacterUpdateData): Promise<Character> {
    return prisma.character.update({ // Use imported `prisma`
      where: { id },
      data,
    });
  }
  ...
  async delete(id: string): Promise<Character> {
    return prisma.character.delete({ // Use imported `prisma`
      where: { id },
    });
  }
  ...
  async count(filter?: Prisma.CharacterWhereInput): Promise<number> {
    return prisma.character.count({ // Use imported `prisma`
      where: filter,
    });
  }

  async nameExists(name: string): Promise<boolean> {
    const count = await prisma.character.count({ // Use imported `prisma`
      where: { name },
    });
    return count > 0;
  }
