...
class EquipmentEntityRepositoryInternal {
	 ...
	 */
	async findByCharacterId(characterId: string, isActive?: boolean, includeDetails: boolean = false): Promise<Array<EquipmentEntityWithDetails | EquipmentEntity>> {
	 ...
	 */
	async findByCharacterAndSlot(characterId: string, slotType: string, includeDetails: boolean = false): Promise<EquipmentEntityWithDetails | EquipmentEntity | null> {
		return prisma.equipmentEntity.findUnique({
			where: {
