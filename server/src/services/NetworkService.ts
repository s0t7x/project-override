...
class NetworkServiceInternal {
	 ...
	 */
	public broadcastMessage(room: { broadcast: (type: string | number, message?: any, options?: any) => void }, message: IMessage, options?: any): void {
	 ...
	 */
	public broadcastError(room: { broadcast: (type: string | number, message?: any, options?: any) => void }, error: Error | ServerError, options?: any, customMessageType?: string): void {
		let payload: IServerErrorMessage;
		if (error instanceof ServerError) {
			payload = { ...error }; // Cast to the shared error type
		} else {
