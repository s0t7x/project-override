...
import {
  EntityComponentsTypeEnum,
  IEntityComponents,
} from '@project-override/shared/dist/components/EntityComponents';
...
export class CharacterSummary extends Schema implements ICharacterSummary {
  ...
  @type({ array: EquipmentVisual }) equipmentVisuals: ArraySchema<EquipmentVisual> =
    new ArraySchema<EquipmentVisual>();
}
...
export async function CharacterSummaryFromDbObject(character: any) {
  ...
  that.lastPlayed =
    Math.floor(Date.parse(character.lastPlayedAt?.toString()) / 1000) || that.lastPlayed;
  try {
    ...
    for (const equipmentEntityRelation of equipment) {
      ...
      if (!entity) continue;
      const equipmentComponent = (entity.components as unknown as IEntityComponents)?.get(
        EntityComponentsTypeEnum.equipment,
      ) as IEquipmentComponent;
