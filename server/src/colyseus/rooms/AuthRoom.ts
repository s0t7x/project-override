...
export class AuthRoom extends Room<UnusedState> {
  onCreate(options: any) {
    ...
    this.onMessage(
      AuthMessageTypeEnum.AuthLoginRequest,
      async (client, message: AuthLoginRequest) => {
        console.log(
          `[AuthRoom ${this.roomId}] Received AuthLoginRequest from ${client.sessionId}:`,
          message,
        );
        if (message.username && message.password) {
          try {
            ...
            console.log(
              `[AuthRoom ${this.roomId}] Authentication successful for ${client.sessionId}.`,
            );
            networkService.sendMessage(client, new AuthLoginResponse(authTokens));
          } catch (error: Error | any) {
            console.error(
              `[AuthRoom ${this.roomId}] Authentication failed for ${client.sessionId}:`,
              error,
            );
          ...
          networkService.sendError(
            client,
            new ValidationError('Username and password are required.') as ServerError,
          );
        }
      },
    );
    ...
    this.onMessage(
      AuthMessageTypeEnum.AuthRegisterRequest,
      async (client, message: AuthRegisterRequest) => {
        console.log(
          `[AuthRoom ${this.roomId}] Received AuthRegisterRequest from ${client.sessionId}:`,
          message,
        );
        if (message.username && message.password) {
          try {
            ...
            console.log(
              `[AuthRoom ${this.roomId}] Registration successful for ${client.sessionId}.`,
            );
            networkService.sendMessage(client, new AuthRegisterResponse());
          } catch (error: any) {
            console.error(
              `[AuthRoom ${this.roomId}] Registration failed for ${client.sessionId}:`,
              error,
            );
    ...
    this.onMessage(
      AuthMessageTypeEnum.AuthRefreshRequest,
      async (client, message: AuthRefreshRequest) => {
        console.log(
          `[AuthRoom ${this.roomId}] Received AuthRefreshRequest from ${client.sessionId}:`,
          message,
        );
        if (message.token) {
          try {
            ...
            console.error(
              `[AuthRoom ${this.roomId}] Refresh failed for ${client.sessionId}:`,
              error,
            );
            networkService.sendError(client, error);
          }
    ...
    this.onMessage('*', (client, type: any, message) => {
      ...
      console.log(
        `[AuthRoom ${this.roomId}] Received message type "${type}" from ${client.sessionId}:`,
        message,
      );
