...
export class CharactersRoom extends Room<CharactersRoomState> {
	...
	onCreate(_options: any) {
		...
		this.onMessage(CharacterMessageTypeEnum.CharacterCreateRequest, async (client, message: CharacterCreateRequest) => {
			...
			try {
				...
				console.log(`[CharactersRoom ${this.roomId}] Client ${client.sessionId} created Character "${char.name}" - ${char.id}.`);
		...
		this.onMessage(CharacterMessageTypeEnum.CharacterDeleteRequest, async (client, message: CharacterDeleteRequest) => {
			if (!client.auth) return;
			const authData = client.auth as IJwtPayload;
			try {
				...
				console.log(`[CharactersRoom ${this.roomId}] Client ${client.sessionId} deleted Character "${char.name}" - ${char.id}.`);
			} catch (error: Error | any) {
				networkService.sendError(client, error instanceof ServerError ? error : new ServerError(error.message));
			}
		});
		...
		this.onMessage('*', (client, type: any, message) => {
			...
			console.log(`[CharactersRoom ${this.roomId}] Received message type "${type}" from ${client.sessionId}:`, message);
	...
	onAuth(client: Client<any, any>, options: any, context: AuthContext) {
		...
		console.log(`[CharactersRoom ${this.roomId}] Client ${client.sessionId} authenticated with user ID ${jwtPayload.userId}.`);
