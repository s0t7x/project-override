...
export class BgmService {
    ...
    public async play(track: BgmTrack, fadeDuration: number = 1000, volume?: number): Promise<void> {
        if (!this.isInitialized) {
            console.error("[BgmService] Play called but not initialized."); return;
        }
        ...
        try {
            ...
            console.warn(`[BgmService] Playback blocked for "${requestedTrackName}" (Error: ${error.name} - ${error.message}). Current volume: ${this.currentAudio?.volume}. Waiting for unlock retry.`);
            // Volume remains 0, unlock handler will retry if needed.
        }
    }

    public async stop(fadeDuration: number = 1000): Promise<void> {
         if (!this.isInitialized) { console.error("[BgmService] Stop called but not initialized."); return; }
    ...
    public setVolume(volume: number, fadeDuration: number = 0): void {
        if (!this.isInitialized) { console.error("[BgmService] setVolume called but not initialized."); return; }
